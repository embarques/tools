...
def init_indexes_cmd(ctx: click.Context):
    ...
    try:
        ...
        for name in ["activitylogs", "invoicedetails", "journals"]:
        ...
        # Branches
        create_unique_index(db, "branches", {"name": 1})

        # Customers
        create_unique_index(db, "customers", {"name": 1, "phone1": 1})

        # Invoices
        create_unique_index(db, "invoices", {"number": 1})

        # Users
        create_unique_index(db, "users", {"userName": 1, "roles": 1})

        # Containers
        create_unique_index(db, "containers", {"name": 1})

        # Accounts
        create_unique_index(db, "accounts", {"name": 1})

        # Income statements
        create_unique_index(db, "incomestatements", {"date": 1, "branch.code": 1})

        # Roles
        create_unique_index(db, "roles", {"name": 1})

        # Permissions
        create_unique_index(db, "permissions", {"name": 1})

        # Cities
        create_unique_index(db, "cities", {"name": 1})
